...
from app.db.models import AgentTrace, ChatSession, Message
...
def run_turn(user_id: int, session_id: int, user_message: str, emit: Emit) -> None:
    ...
    try:
        ...
        assistant = db.get(Message, state.get("assistant_message_id")) if state.get("assistant_message_id") else None
        if assistant is not None:
            assistant.meta = {**(assistant.meta or {}), "trace_id": trace.id,
                              "steps": [{k: n[k] for k in ("node", "label", "ms", "note", "status")} for n in state.get("trace", [])]}
            db.commit()
            emit("reply_done", {"message": message_to_dict(assistant), "trace_id": trace.id, "total_ms": trace.total_ms,
